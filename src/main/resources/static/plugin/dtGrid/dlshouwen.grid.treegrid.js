			...
			var gridObject = {
				...
				fastQuery : function(){
					...
					if(gridReflectionObj.init.fastQueryWindowIsInit){
						$('#fast_query_'+gridReflectionObj.option.id+'_modal').modal('open');
						return;
					}
					var content = '';
					...
					$('#doFastQuery_'+gridReflectionObj.option.id).click(function(){
						...
						$('#fast_query_'+gridReflectionObj.option.id+'_modal').modal('close');
					});
					//显示快速查询
					$('#fast_query_'+gridReflectionObj.option.id+'_modal').modal('open');
					//标识初始化完成
				...
				advanceQuery : function(){
					...
					if(gridReflectionObj.init.advanceQueryWindowIsInit){
						$('#advance_query_'+gridReflectionObj.option.id+'_modal').modal('open');
						return;
					}
					//加载查询内容
					var content = '';
...
//					});
				...
				doAdvanceQuery : function(){
					...
					if(pass){
						...
						gridReflectionObj.load();
						$('#advance_query_'+gridReflectionObj.option.id+'_modal').modal('close');
					}
				},
				...
				print : function(){
					...
					if(gridReflectionObj.init.printWindowIsInit){
						$('#dlshouwen_grid_print_'+gridReflectionObj.option.id+'_modal').modal('open');
						return;
					}
					//放置新的打印选项
					var content = '';
					...
					$('#dlshouwen_grid_print_execute_'+gridReflectionObj.option.id).click(function(){
						...
						$('body').show();
						$('#dlshouwen_grid_print_'+gridReflectionObj.option.id+'_modal').modal('close');
				...
				exportFile : function(exportType){
					...
					if(gridReflectionObj.init.exportWindowIsInit[exportType]){
						//显示导出选项
						$('#dlshouwen_grid_export_'+exportType+'_'+gridReflectionObj.option.id+'_modal').modal('open');
						return;
					}
					//放置新的导出选项
					var content = '';
					...
					content += '				<div class="form-export am-modal-bd form-horizontal form-export">';
					content += '					<div class="form-group">';
					...
					content += '					</div>';
					content += '				</div>';
					content += '				<table class="am-table am-table-bordered am-table-hover table-export">';
					content += '					<thead>';
					content += '						<tr>';
					...
					$('#dlshouwen_grid_export_execute_'+exportType+'_'+gridReflectionObj.option.id).click(function(){
						...
						$('#dlshouwen_grid_export_'+exportType+'_'+gridReflectionObj.option.id+'_modal').modal('close');
					});
					//设置初始化完成
		...
		tools : {
			...
			getWindowStart : function(id, title){
				var content = '';
				content += '<div class="am-modal " id="'+id+'_modal" style="z-index:99999" tabindex="-1" role="dialog" aria-labelledby="'+id+'ModalLabel" aria-hidden="true">';
				content += '	<div class="am-modal-dialog">';
				content += '			<div class="am-modal-hd">';
				content += '				<a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close>&times;</a>';
				content += '				<h4 class="modal-title" id="'+id+'ModalLabel">'+title+'</h4>';
				content += '			</div>';
				content += '		<div class="am-modal-bd">';
				return content;
			},
			//创建一个模态窗口（结束）
			getWindowEnd : function(closeButtonTitle, buttons){
				var content = '';
				content += '			<div class="am-modal-footer">';
				content += '				'+buttons;
				content += '			</div>';
				content += '		</div>';
				content += '	</div>';
...
(function($) {
	$.fn.dlshouwen.grid.defaultOptions = {
		grid : {
			...
			checkWidth : null,
			tableStyle : '',
			tableClass : 'am-table am-table-bordered am-table-hover am-scrollable-horizontal',
			showHeader : true,
